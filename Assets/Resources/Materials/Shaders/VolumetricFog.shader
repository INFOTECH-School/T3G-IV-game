Shader "Custom/VolumetricFog"
{
    Properties
    {
        ...
        _WindSpeed("Wind Speed", Vector) = (0.5, 0.0, 0.5, 0)
    }
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                while(distTravelled < distLimit)
                {
                    ...
                    if (density > 0)
                    {
                        ...
                        {
                            Light light = GetAdditionalLight(lightIndex, rayPos);
                            float attenuation = light.distanceAttenuation * light.shadowAttenuation;
                            fogCol.rgb += light.color.rgb * _LightContribution.rgb 
                                          * henyey_greenstein(dot(rayDir, light.direction), _LightScattering) 
                                          * density * attenuation * _StepSize;
                        }
